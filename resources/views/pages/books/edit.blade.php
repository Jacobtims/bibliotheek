    ...
    <div class="py-12">
        <x-card class="max-w-7xl mx-auto sm:px-6 lg:px-8 py-6 lg:py-8">
            <h1 class="text-2xl font-semibold mb-7">
                ...
                <a href="{{ route('books.index') }}" class="underline">Boeken</a> /
                {{ $book->title }}
            </h1>

            <form method="POST" action="{{ route('books.update', $book->id) }}" class="w-full md:w-1/2">
                @csrf @method('PUT')

                <div>
                    <x-input-label for="isbn" value="ISBN" />
                    <x-text-input id="isbn" class="block mt-1 w-full" type="text" name="isbn" placeholder="ISBN" :value="old('isbn', $book->isbn)" required autofocus/>
                    <x-input-error :messages="$errors->get('isbn')" class="mt-2" />
                </div>

                <div class="mt-4">
                    <x-input-label for="title" value="Titel" />
                    <x-text-input id="title" class="block mt-1 w-full" type="text" name="title" placeholder="Titel" :value="old('title', $book->title)" required/>
                    <x-input-error :messages="$errors->get('title')" class="mt-2" />
                </div>

                <div class="mt-4">
                    <x-input-label for="image" value="Afbeelding url" />
                    <x-text-input id="image" class="block mt-1 w-full" type="url" name="image" placeholder="Afbeelding url" :value="old('image', $book->image)" required/>
                ...
                <div class="mt-4">
                    ...
                    <x-text-input id="purchased_at" class="block mt-1 w-full" type="date" name="purchased_at" placeholder="Gekocht op" :value="old('purchased_at', $book->purchased_at)" required/>
